...
void prepare_list_user(char *info) {
    print_list_in_buffer(users_list, info);
    return;
}
...
bool try_to_challenge(char *adversary_nickname, node *node_of_guest, uint16_t *adversary_port) {
    pthread_mutex_lock(&mutex_list_users);
    node *node_of_adversary = get_node_by_nickname(users_list, adversary_nickname);
...
bool check_pending_request(node *node_of_guest, int sock) {
    int guest_answer;
    ...
    if (set_socket_blocking_enabled(sock, true) == false) {
    ...
    // printf("[%s]: It should be asked guest --> 1/0? ",node_of_guest->nickname); ///MAYBE USE ANOTHER SOCKET NOT BLOCKING FOR THE CLIENT SO THAT
    // PERIODICALLY CHECK IF SOMEONE HAS REQUESTED TO PLAY WITH HIM scanf("%d",&guest_answer);
    //
    pthread_mutex_lock(&mutex_list_users);
    ...
    if (set_socket_blocking_enabled(sock, false) == false) {
...
void *thread_handler_client(void *ptr) {
    ...
    int len;
    bool result;
    char guest_nickname[NICKNAME_LENGTH];  // WILL BE OBTAINED BY FILE IN WHICH THERE ARE STORED PUBKEYS
    node *node_of_guest;
    ...
    */
    ...
    if (name == 0) {
        ...
        node_of_guest = insert_first(&users_list, (long)pthread_self(), "graziano", user_address);
    }
    if (name == 1) {
        strncpy(guest_nickname, "silvano", NICKNAME_LENGTH);
        node_of_guest = insert_first(&users_list, (long)pthread_self(), "silvano", user_address);
    }
    if (name == 2) {
        strncpy(guest_nickname, "loriano", NICKNAME_LENGTH);
        node_of_guest = insert_first(&users_list, (long)pthread_self(), "loriano", user_address);
    }
    if (name == 3) {
        strncpy(guest_nickname, "oreste", NICKNAME_LENGTH);
        node_of_guest = insert_first(&users_list, (long)pthread_self(), "oreste", user_address);
    }
    name++;
    pthread_mutex_unlock(&mutex_list_users);
    ...
    while (1) {
        ...
        */
        if (set_socket_blocking_enabled(conn->sock, false) == false) {
        ...
        } while (read(conn->sock, &command_int, sizeof(int)) <= 0);

        printf("[%s]: we've received -> %d\n", guest_nickname, command_int);

        // Come back to blocking socket
        if (set_socket_blocking_enabled(conn->sock, true) == false) {
            printf("[%s]: Error in setting blocking socket\nI've to abort!", guest_nickname);
        }

        //!!! HANDLE LIST TOO LONG
        //send [OPCODE | PAYLOAD_BYTES | PAYLOAD(LIST)]
        if (command_int == MSG_PROTOCOL_SHOW_LIST) {
            ...
            if (result == true) {
                ...
                do {
                    ...
                    read(conn->sock, &command_int, sizeof(int));
                } while (command_int != 7);
        ...
        } else if (command_int == 6) {  // THE CLIENT HAS ALERT THE SERVER THAT HE'S PLAYING P2P
            do {
                printf("[%s]: Waiting for the end of the game\n", guest_nickname);
        ...
        } else if (command_int == 8) {  // THE CLIENT INFORMs ITS SERVER THREAD ON WHICH PORT HE WILL LISTEN FOR P2P GAMING
            int client_p2p_port;
    ...
    // PROTECT WITH MUTEX!
    pthread_mutex_lock(&mutex_list_users);
    delete_elem(&users_list, (long)pthread_self());
    pthread_mutex_unlock(&mutex_list_users);
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        // check if accept has returned a valid socketID
