...
#define M2_CLIENT_SERVER_AUTH \
    101  // |101|len|Cs_len Cs Challenge_A ID_SERVER ID_CLIENT Challenge_S Yserv Sign_size EprivKeyServer(ID_SERVER ID_CLIENT Challenge_S Yserv)
         // //send the mex also in clear, then verify
#define M3_CLIENT_SERVER_AUTH \
    102  // |102|len|ID_CLIENT ID_SERVER Challenge_A Yclient_len Yclient Sign_size EprivKeyClient(ID_CLIENT ID_SERVER Challenge_A Yclient_len
         // Yclient)|
...
#define M1_INFORM_SERVER_GAME_START \
    115  // |115|len|EKas(NONCE_CLIENT) //NONCE_CLIENT will be send againg by client to avoid replay attack to inform erroneously the server about the
         // ending of the game
...
#define M2_CLIENT_CLIENT_AUTH \
    122  // |122|len|Challenge_M ID_SLAVE ID_MASTER Challenge_S Yslave_len Yslave Sign_size EprivKeyServer(ID_SLAVE ID_MASTER Challenge_S Yslave_len
         // Yslave)
#define M3_CLIENT_CLIENT_AUTH \
    123  // |123|len|ID_CLIENT ID_SERVER Challenge_A Yclient_len Yclient Sign_size EprivKeyClient(ID_CLIENT ID_SERVER Challenge_A Yclient_len
         // Yclient)|
...
#define M_CLOSE \
    126  // |120|len|EKas(Kas) //No worry about replay since for definition only once sent (Kas is to add something otherwise if only opcode everybody
         // could send it to ruin the game)
...
Message* create_M_REQ_ACCEPT_PLAY_TO_ACK(char* username_opponent,
                                         AuthenticationInstance* authInstance);  // from master: to ask guest if wants to play
...
int handler_M2_CLIENT_CLIENT_AUTH(unsigned char* payload, unsigned int payload_len, AuthenticationInstanceToPlay* authInstanceToPlay,
                                  EVP_PKEY* prvkey);
